# ASpace_API_Workshop
These are the resources used in the "Introduction to Working with the ArchivesSpace API" workshop presented at the 2017 ArchivesSpace Member Forum in Portland, Oregon. Note that these are the scripts used in the workshop itself.
...
* *Windows users*
   ...
   Windows users who attended the workshop will recall that we took steps to install _cygwin_, which is a "Linux-like environment for Windows making it possible to port software running on POSIX systems (such as Linux, BSD, and Unix systems) to Windows." Please see our presentation slides to walk yourself through installing cygwin and the packages required to run our scripts. Keep the cygwin installer around, it comes in handy (see immediately below).
...
repository='YOURREPOSITORYNUMBER'
```
